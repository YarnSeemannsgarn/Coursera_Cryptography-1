...
MSGS = ["Bob sends a message to Alice",
        "Alice is the woman of ciphertexts",
        "This program is the best of the world"]
...
def main():
    ...
    # All m1 xor m2
    m1m2s = []
    for ctr1, cipher1 in enumerate(ciphertexts):
        print "Length of cypher" + str(ctr1) + ": " + str(len(cipher1))
        for ctr2, cipher2 in enumerate(ciphertexts[ctr1+1:]):
            tup = ctr1, ctr1+ctr2+1, strxor(cipher1, cipher2)
            m1m2s.append(tup)

    # Try to break with known plain
    knownPlain = "We can factor the number 15 with quantum computers. We can factor the number 15 with a dog trained to bark three times - Robert Harley"
    for m1m2 in m1m2s:
        cipher1 = m1m2[0]
        cipher2 = m1m2[1]
        m1xorm2 = m1m2[2]
        for i in range(0, len(m1xorm2) - len(knownPlain)):
            word = "".join(chr(ord(x) ^ ord(y)) for (x, y) in zip(m1xorm2[i:i+len(knownPlain)], knownPlain))
            print str(cipher1).ljust(2), str(cipher2).ljust(2), str(i).ljust(5), word
    """
